...
def main(data_dir: str | Path):
    ...
    predicted_data = np.load(data_dir / "predicted_weights.npz")
    labels_data = np.load(data_dir / "labels_weights.npz")
    ...
    for _ in tqdm.tqdm(range(1000)):
        ...
        writer = pd.ExcelWriter(data_dir / "f1_bootstrap.xlsx", engine="xlsxwriter")
        concatenated_df_f1.to_excel(writer, sheet_name="Sheet1", index=False)
        writer.close()

        writer = pd.ExcelWriter(data_dir / "acc_bootstrap.xlsx", engine="xlsxwriter")
        concatenated_df_acc.to_excel(writer, sheet_name="Sheet1", index=False)
        writer.close()

        writer = pd.ExcelWriter(
            data_dir / "precision_bootstrap.xlsx", engine="xlsxwriter"
        )
