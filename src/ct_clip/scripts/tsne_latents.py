...
def plot_tsne(embedding, labels, k):
    ...
    plt.ylabel("t-SNE 2")
    plt.savefig(
        f"tsne_plots_test2/image_diseases/{name}_healthyornot_text.png", dpi=600
    )
    plt.show()
    plt.clf()
...
if __name__ == "__main__":
    ...
    for i in tqdm.tqdm(range(18)):
        ...
        validation_labels_sum = map_accessions_to_labels(
            validation_accessions, csv_path, i, isValid=True
        )

        combined_labels_sum = np.concatenate([train_labels_sum, validation_labels_sum])

        train_path1 = (
            "path_to_train_image_latents.npz"  ##could be done for text as well
        )
        validation_path = (
            "path_to_valid_image_latents.npz"  ##could be done for text as well
        )

        if i == 0:
            all_latents = load_and_concatenate(
                train_path1, train_path2, validation_path
            )
